...
_DEFAULT_FRAGSTORE_DIR = os.path.join(
    os.path.dirname(os.path.realpath(__file__)),
    "../data/fragment_libraries",
)
_DEFAULT_POPULATION_SMILES_DIR = os.path.join(
    os.path.dirname(os.path.realpath(__file__)),
    "../data/smiles_files",
)
DEFAULT_FRAGSTORE_PATH = os.path.join(
    _DEFAULT_FRAGSTORE_DIR,
    "chembl_33_chemreps_std_fragstore_brics_filter2.pkl",
)
DEFAULT_STARTING_POPULATION_SMILES_PATH = os.path.join(
    _DEFAULT_POPULATION_SMILES_DIR,
    "chembl_33_chemreps_std.smiles",
)
...
class FragGTGenerator:
    def __init__(
        ...
        # map-elites (frag-gti)
        self.map_elites = (
            None if map_elites is None else map_elites_factory(map_elites, fragmentation_scheme)
        )
    ...
    def write_generation_results(
        ...
        df.to_csv(f"{all_generations_results_dir}/{safe_job_name!s}_{generation + 1}.csv")
    ...
    def optimize(
        ...
        if starting_population is None:
            ...
            logger.info(
                f"using user provided initial population for generation: {starting_population}",
            )
        ...
        population_scores = scoring_function.score_list(
            [Chem.MolToSmiles(x) for x in initial_population],
        )
        ...
        for generation in range(self.generations):
            ...
            new_mol_tuples = [
                (m, s) for m, s in new_population_tuples if s not in existing_population_smiles
            ]
            if not new_mol_tuples:
                patience += 1
            ...
            logger.debug(
                f"{len(new_population) - len(new_molecules)} smiles already existed in the population",
            )
