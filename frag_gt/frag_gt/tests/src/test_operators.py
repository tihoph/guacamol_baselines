...
from frag_gt.src.operators import (
    add_node_mutation,
    connect_mol_from_frags,
    delete_node_mutation,
    single_point_crossover,
    substitute_edge_mutation,
    substitute_node_mutation,
)
...
def test_connect_mol_from_frags_brics() -> None:
    ...
    # When
    reconstructed_mols = [
        connect_mol_from_frags(frags, fragmentor=BRICS_FRAGMENTOR) for frags in mollist_frags
    ]

    # Then
    for x, y in zip(
        [Chem.MolToSmiles(m) for m in mollist],
        [Chem.MolToSmiles(m) for m in reconstructed_mols],
    ):
        assert x == y
