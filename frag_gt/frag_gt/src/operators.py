...
def connect_mol_from_frags(
    ...
    for match in composite_frags.GetSubstructMatches(DUMMY_PLUS_ANCHOR_SMARTS):
        ...
        new_bond_info[
            1
        ].sort()  # sort isotopes ascending to match keys in fragmentor.recombination_rules
    ...
    for i in pairs:
        new_mol.AddBond(
            pairs[i][0][0],
            pairs[i][0][1],
            recombination_dict[(str(pairs[i][1][0]), str(pairs[i][1][1]))],
        )
...
def _get_partner_cut(added_cut_type_new_frag: str, fragmentor: FragmentorBase) -> str:
    ...
    """
    possible_joins = [
        rule for rule in fragmentor.recombination_rules if added_cut_type_new_frag in rule
    ]
...
def delete_node_mutation(
    ...
    if not strict:
        ...
        if len(gene_type_results) > 1:
            raise RuntimeError(
                f"MutationDelNode: DB is corrupted, multiple results for gene_type {gene_type}",
            )
        if gene_type == "":
            ...
            logger.debug(
                f"MutationDelNode: deleting frag from {Chem.MolToSmiles(parent_mol)} "
            ...
            logger.debug(
                f"MutationDelNode: hanging frag gene type ({gene_type}) not in DB (strict behaviour)",
            )
            frag_exists = False
        else:
        ...
        if frag_exists:
            ...
            if not len(mutant_smiles):
                logger.debug(
                    f"MutationDelNode: no frags with {gene_type} to mutate hanging frag to",
                )
                return []
            ...
            while (accept_mutant is False) and (len(mutant_smiles) > 0):
                ...
                if query_idx_types != mutant_idx_types:
                    accept_mutant = False
                    logger.debug(
                        f"MutationDelNode: Not matching types: {query_idx_types} {mutant_idx_types}",
                    )
                    logger.debug(
                        f"MutationDelNode: {Chem.MolToSmiles(parent_mol)} {Chem.MolToSmiles(query_frag)} "
...
def substitute_node_mutation(
    ...
    while (attempt < 5) and (accept_mutant is False):
        ...
        if Chem.MolToSmiles(mutant_frag) == Chem.MolToSmiles(query_frag):
            logger.debug("MutationSubNode: skipping frag as its same as query")
            continue
        ...
        if query_idx_types != mutant_idx_types:
            accept_mutant = False
            logger.debug(
                f"MutationSubNode: Not matching types: {query_idx_types} {mutant_idx_types}",
            )
            logger.debug(
                f"MutationSubNode: {Chem.MolToSmiles(parent_mol)} {Chem.MolToSmiles(query_frag)} "
                f"{Chem.MolToSmiles(mutant_frag)}",
            )
        else:
            accept_mutant = True
...
def add_node_mutation(
    ...
    while (accept_mutant is False) and (len(cut_types) > 0):
        ...
        pendant_smiles_list, _ = frag_db.query_frags(
            added_cut_type_new_frag,
        ...
        if len(pendant_smiles_list):
            ...
            logger.debug(
                f"MutationAddNode: No small fragments in DB with cut_type: {added_cut_type_new_frag}",
            )
            continue
        ...
        mutant_gene_type_list = sorted(
            [int(x) for x in gene_type_list if x != ""] + [int(added_cut_type_existing_frag)],
        )
        ...
        mutant_smiles, mutant_scores = frag_db.query_frags(
            mutant_gene_type,
            query_frag,
            x_choices=0.3,
        )
        ...
        while (accept_mutant is False) and (len(mutant_smiles) > 0):
            ...
            if len(difference) == 1 and int(difference.pop()[1]) == -1:
                ...
                accept_mutant = False
                logger.debug(
                    f"MutationAddNode: Not matching types: {query_idx_types} {mutant_idx_types}",
                )
                logger.debug(
                    f"MutationAddNode: {Chem.MolToSmiles(parent_mol)} {Chem.MolToSmiles(query_frag)} "
                    f"{Chem.MolToSmiles(mutant_frag)}",
                )
    ...
    if accept_mutant:
        ...
        return [new_mol]
    logger.debug(
        f"MutationAddNode: no suitable additions were found for {Chem.MolToSmiles(query_frag)}",
    )
    return []
...
def single_point_crossover(
    ...
    if not common_cts:
        logger.debug(
            f"CrossoverSP: no common cut types: {Chem.MolToSmiles(m1)}, {Chem.MolToSmiles(m2)}",
        )
        return []
    ...
    try:
        return [
            connect_mol_from_frags(new_m1, fragmentor=fragmentor),
...
def mc_operator_factory(operator_name: str):
    ...
    if operator_name.lower() == "add_node_mutation":
        return add_node_mutation
    if operator_name.lower() == "delete_node_mutation":
        return delete_node_mutation
    if operator_name.lower() == "substitute_edge_mutation":
        return substitute_edge_mutation
    if operator_name.lower() == "single_point_crossover":
