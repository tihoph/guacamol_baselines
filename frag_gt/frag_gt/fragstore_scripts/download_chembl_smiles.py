...
def get_argparser():
    ...
    parser.add_argument(
        ...
        "--chembl_version",
        default="chembl_33",
        help="chembl database version (default: 33)",
    )
    parser.add_argument(
        "-d",
        "--data_root",
        default=".",
        help="root to dir containing raw chembl smiles download",
    )
    parser.add_argument(
        "-s",
        "--standardize_mols",
...
def download_chembl_smiles():
    ...
    if args.standardize_mols:
        ...
        print(
            f"Processed {len(raw_smiles)} raw smiles to produce {len(std_smiles)} standardized outputs",
        )
    else:
    ...
    output_filename = os.path.join(
        args.data_root,
        chembl_chemreps_filename.replace(".txt.gz", "_std.smiles"),
    )
